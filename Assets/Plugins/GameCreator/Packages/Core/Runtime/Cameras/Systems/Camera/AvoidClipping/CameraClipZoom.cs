...
namespace GameCreator.Runtime.Cameras
{
    ...
    public class CameraClipZoom : TCameraClip
    {
        ...
        {
            if (target == null) return camera.transform.position;
            if (!camera.Transition.CurrentShotCamera.AvoidClipping) return camera.transform.position;
            if (camera.transform.position == target.position) return camera.transform.position;
        ...
        private bool IsChild(Transform hit, Transform[] ignoreList)
        {
