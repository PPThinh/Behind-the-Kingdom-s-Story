...
{
    internal class ManipulatorBlackboardSort : MouseManipulator
    {
        // MEMBERS: -------------------------------------------------------------------------------
        ...
        private ParameterTool m_CurrentParameter;
        
        // PROPERTIES: ----------------------------------------------------------------------------

        public ParameterTool DraggedParameter => this.m_IsDragging
            ? this.m_CurrentParameter
            : null;

        // INITIALIZERS: --------------------------------------------------------------------------

        public ManipulatorBlackboardSort(ParametersTool parent)
        {
        ...
        private int ClosestElementIndex(float cursor)
        {
            ...
            List<ParameterTool> parameters = this.m_Parent.Parameters;
            
            for (int i = 0; i < parameters.Count; ++i)
            {
                ParameterTool parameter = parameters[i];
        ...
        private void OnMouseDown(MouseDownEvent eventMouseDown)
        {
            ...
            this.m_CurrentParameter = GetAncestorOfType<ParameterTool>(this.m_CurrentTarget);
            
            this.m_StartIndex = this.m_Parent.GetParameterIndex(this.m_CurrentParameter);
        ...
        private void OnMouseUp(MouseUpEvent eventMouseUp)
        {
            ...
            {
                this.m_Parent.MoveParameters(this.m_StartIndex, this.m_CurrentIndex);
            }
            else
            {
