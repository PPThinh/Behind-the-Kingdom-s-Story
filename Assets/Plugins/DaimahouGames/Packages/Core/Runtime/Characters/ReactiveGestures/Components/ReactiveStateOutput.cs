...
namespace DaimahouGames.Runtime.Characters
{
    public class ReactiveStateOutput : MonoBehaviour
    {
        ...
        public async Task SetState(ReactiveState state, Args args)
        {
            ...
            var configuration = new ConfigState(
                0, m_CurrentState.GetSpeed(args), 1,
                0.15f,0.15f
            );
            
            var task = m_Character.States.SetState(state, 0, BlendMode.Blend, configuration);
        ...
        public void StopState(Args args)
        {
            ...
            m_Character.States.Stop(0, 0, 0.15f);
            m_CurrentState = null;
        }
